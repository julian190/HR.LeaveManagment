...
namespace HR.LeaveManagment.Identity.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<AuthResponse> Login(AuthRequest request)
        {
            ...
            if (!result.Succeeded)
            {
                throw new Exception($"Credentials for {request.Email} aren't vaild");
            }
        ...
        public async Task<RegistrationResponse> Register(RegistrationRequest request)
        {
            ...
            {
                throw new Exception($"UserName {request.UserName} Already Exisist");
            }
        ...
        private async Task <JwtSecurityToken> GenerateToken(ApplicationUser user)
        {
            ...
            .Union(roleClaims);
            var symmetricSecuirtyKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_jwtSettings.Key));
